...
class BouncyNestedScrollView @JvmOverloads constructor(
    ...
    interface OnScrollChangeListener {
        ...
        fun onScrollChange(
            v: BouncyNestedScrollView?, scrollX: Int, scrollY: Int,
    ...
    fun overScrollByCompat(deltaY: Int,
        scrollX: Int, scrollY: Int,
        scrollRangeY: Int,
        mX: Int, mY: Int,
    ...
    private fun ensureGlows() {
        if (overScrollMode != OVER_SCROLL_NEVER) {
            if (mEdgeGlowTop == null) {
                val context = context
                mEdgeGlowTop = BouncyEdgeEffect(context, spring, this, EdgeEffectFactory.DIRECTION_TOP)
                mEdgeGlowBottom = BouncyEdgeEffect(context, spring, this, EdgeEffectFactory.DIRECTION_BOTTOM)
            }
        } else {
            mEdgeGlowTop = null
    ...
    internal class AccessibilityDelegate : AccessibilityDelegateCompat()
    {
        override fun performAccessibilityAction(host: View, action: Int, arguments: Bundle): Boolean
        {
            ...
            when (action)
            {
                ...
                {
                    run{
                        val viewportHeight: Int = (nsvHost.height - nsvHost.paddingBottom
                                - nsvHost.paddingTop)
                        val targetScrollY: Int = min(
    ...
    {
        initScrollView()
        val a = context.obtainStyledAttributes(attrs, SCROLLVIEW_STYLEABLE, defStyleAttr, 0)
